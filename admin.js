...
let headerSellerStatusText = null; // Reference to the seller status text in the main header (passed from script.js)
...
// NEW: Store Settings DOM elements
const adminStoreSettings = document.getElementById('admin-store-settings'); // Reference to the store settings tab content div
let adminSellerStatusDisplay = document.getElementById('admin-seller-status-display'); // Element to show status in admin panel
let toggleSellerStatusBtn = document.getElementById('toggle-seller-status-btn'); // Button to toggle status
...
// NEW: Firestore path for store settings
const STORE_SETTINGS_DOC_PATH = `artifacts/${APP_ID}/storeSettings/config`;
...
export function initAdminPanel(db, auth, userId, adminFlag, headerSellerStatusTextElement) {
    ...
    headerSellerStatusText = headerSellerStatusTextElement; // Assign the passed element from script.js

    // Re-assign DOM elements to ensure they are found after dynamic import
    // (Though they are global constants, re-assigning them within this function
    //  helps ensure they are correctly referenced after the script fully loads/executes)
    ...
    // --- Store Settings Event Listeners ---
...
function showAdminTab(tabName) {
    ...
    adminStoreSettings.style.display = 'none'; // Ensure store settings is hidden by default
